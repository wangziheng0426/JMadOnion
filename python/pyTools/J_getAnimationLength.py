...
import J_exportExcel
reload(sys)
sys.setdefaultencoding('utf-8')
from PyQt4 import QtGui
from PyQt4 import QtCore

class J_exportExcelToDev(QtGui.QMainWindow, J_exportExcel.Ui_MainWindow):
    def __init__(self):
        super(J_exportExcelToDev, self).__init__()
        self.setupUi(self)
        self.J_createSlots()

    def readLength(self,_animFile):
    ...
    def readExcel(self,excelFile):
        ...
        return excelDic
    def findFileInDesk(self,AssetsPath,state=2):
        modelPath=AssetsPath+"/GameResources/models"
        logStr=""
        count=0
        if state == 0 or state==2:
            excelDic=self.readExcel(AssetsPath + '/DevResources/ExcelConfig/ModelAnim.xls')
            for key in excelDic:
        ...
        if state==1 or state==2:
            excelDic = self.readExcel(AssetsPath + '/DevResources/ExcelConfig/FacialAnim.xls')
            for key in excelDic:
                rolePath=modelPath+'/'+key.encode('gbk')
    ...
    def fillExcel(self,AssetsPath,state=2):
                ...
                if os.path.isdir(gameModelPath+"/"+m):
                    ...
                    for ani in animationFiles:
                        if ani.endswith(".anim"):
                            aniDic={}
                            aniDic[ani]=self.readLength(animationPath+'/'+ani)
                            newExcelDic[m.lower()].append(aniDic)
            self.writeToAnimationExcel(newExcelDic, excelFile)
                ...
                if os.path.isdir(gameModelPath + "/" + m):
                    ...
                    for ani in animationFiles:
                        if ani.endswith(".anim"):
                            ...
                            aniDic[ani] = self.readLength(animationPath + '/' + ani)
                            newExcelDic[m.lower()].append(aniDic)
            self.writeToFacialExcel(newExcelDic, excelFile)
            print (u'用时：' + str(time.time() - tt) + u"秒")
    def writeToAnimationExcel(self,dic,filePath):
        index=3
        ...
        for key in sorted(dic):
            for aniDic in sorted(dic[key]):
                for keyAni in aniDic:
                    ...
                    index=index+1
        book.save(filePath)
    def writeToFacialExcel(self,dic, filePath):
        index = 3
        book = xlwt.Workbook(encoding='utf-8', style_compression=0)
    ...
    #readInfo(ur"C:/main/Assets/GameResources/models/c_cl_pinghai/facial/retreat.anim")
        book.save(filePath)
    def buttonClickProc(self,state):
        assetsPath=str(self.lineEdit_inPath.text()).lower().replace('\\','/')

        self.fillExcel(assetsPath.decode('utf-8'), state)


    ################################ 链接按钮
    def J_createSlots(self):
        self.pushButton.clicked.connect(functools.partial(self.buttonClickProc,1))
        self.pushButton_2.clicked.connect(functools.partial(self.buttonClickProc, 0))
        self.pushButton_3.clicked.connect(functools.partial(self.buttonClickProc, 2))
    '''
